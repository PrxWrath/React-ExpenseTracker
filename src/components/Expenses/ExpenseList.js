...
const ExpenseList = (props) => {
    ...
    const deleteExpenseHandler = async(expense) => {
      setLoading(true);
      const res = await fetch(`https://expense-tracker-4ce23-default-rtdb.firebaseio.com/expenses${email}/${expense}.json`,{
                            method:'DELETE',
                            headers:{
    ...
    <>
      ...
      <ListGroup className="w-50 mx-auto mt-4 p-2">
        {Object.values(expenses).map((expense) => {
          return( 
            <ListGroup.Item key={expense.id} className="d-flex justify-content-between border border-success my-1 rounded">
                <h5 className="text-success mx-1 w-25">Rs. {expense.amount}</h5>
                <h5 className="mx-1 w-25">--{expense.category}--</h5>
                <p className="mx-1">{`(${expense.description})`}</p>
                <div className="d-flex ms-auto h-50">
                    ...
                    <Button variant="danger" size="sm" className="mx-1" onClick={()=>{deleteExpenseHandler(expense.id)}}>
