...
const Expenses = () => {
  ...
  useEffect(()=>{
    ...
    if(!expense.loaded)
      changeExpenses();
  },[email, totalAmount, expense, dispatch]);
...
//load from backend
    ...
    async function loadExpenses(){
        ...
        if(res.ok){
          if(data!==undefined&&data!==null)
            dispatch(expenseActions.load({total:data.total, expenses:Object.values(data.expenses)}))   
        }
        setLoading(false);
    }  
    loadExpenses()
  ...
  useEffect(()=>{
    ...
    setCsvData(expense.expenses);
  },[expense.expenses, totalAmount]);
  ...
  return (
    <Container style={{paddingTop:'5rem'}}>
        ...
        <ExpenseForm edited={edited} setEdited={setEdited}/>
        {loading&&<Loader className='my-3'/>}
        <ExpenseList setEdited={setEdited}/>
    </Container>
  )
}
